...
export const loader: LoaderFunction = async ({ request }) => {
  ...
  const idExtension = "4382011361047714530";
  const parsed = JSON.parse(cleaned);
  const blocks = parsed.current.blocks;
  const firstBlock = Object.values(blocks)[0];
  const isDisabled = firstBlock.disabled;
  // console.log(
  //   "responseJson",
  //   JSON.stringify(responseJson.data.appInstallation.metafields),
  // );
...
export default function AppReviews() {
  const { isDisabled } = useLoaderData<typeof loader>();
  const url =
