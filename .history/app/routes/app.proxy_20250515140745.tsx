...
export const action: ActionFunction = async ({ request }) => {
  try {
    ...
    query {
      appInstallation {
        metafields(first: 3, namespace: "product_reviews", keys: ["reviews_${productId}", "rating_${productId}", "count_${productId}"]) {
          edges {
            node {
              id
              key
    ...
    const reviewsNode = metafieldNodes.find(
      (edge) => edge.node.key === `reviews_${productId}`,
    );
    if (reviewsNode) {
      try {
    ...
    // Tìm rating metafield
    const ratingNode = metafieldNodes.find(
      (edge) => edge.node.key === `rating_${productId}`,
    );
    if (ratingNode) {
      try {
        currentRating = parseFloat(ratingNode.node.value);
      } catch (e) {
    ...
    // Tìm count metafield
    const countNode = metafieldNodes.find(
      (edge) => edge.node.key === `count_${productId}`,
    );
    if (countNode) {
      try {
        reviewCount = parseInt(countNode.node.value);
      } catch (e) {
        console.error("Lỗi khi parse count:", e);
      }
    }
      ...
      `,
      {
        variables: {
          metafields: metafieldsToSet.map((field) => ({
            ...field,
            ownerId: "gid://shopify/AppInstallation/1", // Thay ID này bằng ID của app installation của bạn
          })),
        },
      },
    );
