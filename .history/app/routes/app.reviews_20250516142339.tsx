...
export const loader: LoaderFunction = async ({ request }) => {
  ...
  const ratingList = reviewList.filter((item: any) =>
    item.key.startsWith("rating_"),
  );
  return { reviewList, ratingList };
};
...
export default function AppReviews() {
  ...
  // };
  const { reviewsData, ratingList } = useLoaderData<typeof loader>();
  const handleViewDetails = (id: string) => {
    console.log("View details for product ID:", id);
  };
  const listReviews = reviewsData.map((item) => {
    return {
      product: item.key.replace("reviews_", ""),
      rating: ratingList.find((rating) => rating.key === item.key)?.value,
      count: item.value.length,
    };
  });

  const rows = listReviews.map((item) => [
    <Text
      key={`product-${item.id}`}
      variant="bodyMd"
      fontWeight="bold"
